...
open class GBFSearch{
    ...
    class var sharedInstance :GBFSearch {
        struct Singleton {
            static let instance = GBFSearch()
        }
        ...
        while(!q.isEmpty() && !isStopped){
            ...
            if (!tempNode!.getCurState().isGoal()){
                ...
                for i in (0 ..< tempSuccessors.count) {
                    let checkedNode = SearchNode(prev: tempNode!, s: tempSuccessors[i], c: tempNode!.getCost() + tempSuccessors[i].findCost(), h: Double((tempSuccessors[i] as! PuzzleState).getOutOfPlace()), d:0)
                ...
                for i in (0 ..< nodeSuccessors.count){
                    if lowestNode.getHCost() > nodeSuccessors[i].getHCost(){
                        lowestNode = nodeSuccessors[i]
                    }
                }
                
                let lowestValue = lowestNode.getHCost()
                
                for i in (0 ..< nodeSuccessors.count)
                {
                    if (nodeSuccessors[i].getHCost() == lowestValue)
                    {
